...
const YearMonthForm = (props) => {
 ...
 class DatePicker extends PureComponent {
  ...
  componentDidUpdate(prevProps, prevState) {
    const {
      isDayPickerOpen: prevIsDayPickerOpen,
    } = prevState;
    const {
      isDayPickerOpen,
    } = this.state;
  ...
  getDisabledDates = () => {
    ...
    const maximumDisabledDate = getDateRange(new Date(), futureDisabledDaysCount)
    ...
    if (isWeekendDisabled) {
      daysTobeDisabled = [...daysTobeDisabled, {
        daysOfWeek: [0, 6],
      }]
    }
  ...
  adjustTabIndex = () => {
    ...
    if (isDayPickerOpen) {
      document.getElementsByClassName('DayPicker-NavButton DayPicker-NavButton--next')[0].tabIndex = 7;

      let dateBox = document.getElementsByClassName('DayPicker')[0];
      let wrapper = document.getElementsByClassName('DayPicker-wrapper')[0];
      const monthDropdown = document.getElementsByClassName('month-year-dropdown')[0];
      const yearDropdown = document.getElementsByClassName('month-year-dropdown')[1];
      ...
      if (dateBox !== undefined && isDayPickerOpen) {
        ...
        document.addEventListener('click', (event) => {
          if (!dateBox.contains(event.target)
            && !inputBox.contains(event.target) && !wrapper.contains(event.target)) {
            this.hideDayPicker();
          }
        })
      }
